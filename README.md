...
```bash
mvn -B verify
docker build -t isb/isb-payment-execution:0.1.0 .
```

## Deploy (Helm → K8s with Cilium mesh)

```bash
helm upgrade --install isb-payment-execution ./helm -n isb-operations
```

Exposed through the platform Gateway at path prefix `/isb-payment-execution` (Cilium Gateway API). Mesh policy (`CiliumNetworkPolicy`) allows: gateway ingress, same-area peers, Prometheus — everything else denied.
